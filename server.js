...
// ========= KSPSignController (JSON proxy, like PHP controller) =========

const KSP_TARGET_URL =
  "https://id.gov.ua/sign-widget/v20240221/Server/KSPSignController.php";

app.options("/Server/KSPSignController.php", (req, res) => {
  res.status(204).set({
    "Access-Control-Allow-Origin": "*",
...
app.post("/Server/KSPSignController.php", (req, res) => {
  // IMPORTANT: return JSON (not base64)
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "POST, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type",
  ...
  const upstreamReq = https.request(
    KSP_TARGET_URL,
    {
      ...
      // ✅ так само як ти робила в PHP: verify SSL via CA bundle
      // Якщо в тебе на сервері НЕ треба CA file — можеш прибрати ca/cafile і залишити default.
      rejectUnauthorized: true,
      // 1) спробуємо використати env, щоб і на Render працювало без хардкоду шляхів
      ca: (() => {
        const caPath = process.env.CA_BUNDLE_PATH;
  ...
  upstreamReq.on("error", (err) => {
    return res.status(502).json({
      data: null,
      error: { code: 502, message: `Upstream request failed: ${err.message}` },
    });
  });
